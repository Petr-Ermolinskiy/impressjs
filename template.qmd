...
You can have r output on slides (even Plotly).
...
p <- plot_ly(
  data = plot_data,  # Explicitly specify the data parameter
  x = ~disp,
  y = ~reorder(car, disp),
  type = "bar",
  orientation = 'h',
  marker = list(color = ~disp, colorscale = 'Viridis')
) %>%
  layout(
    title = "Displacement by Car",
...
saveWidget(p, "figs/plotly_figure.html", selfcontained = TRUE, libdir = NULL)

# Embed via iframe
cat('
<iframe src="figs/plotly_figure.html"
width="100%"
height="550px"
style="border:none; background: transparent; border-radius: 20px; overflow: hidden;"></iframe>')
```

:::

::: {#slidebehindslide1a .step .slide .center-content data-y="-1000" data-scale=".1" data-z="-1000"}
...
mtcars |> 
  ...
  coord_flip() +
  geom_text(aes(angle = angle), size = 2, hjust = -.1, color = "white") +
  theme_minimal() +
  ...
  theme(
    ...
    # White grid lines with transparency
    panel.grid.major = element_line(color = "#717171", linewidth = 0.5),
    panel.grid.minor = element_line(color = "#717171", linewidth = 0.3),
